...
export default function Categories({handleSidebar, showSidebar}){
    ...
    const [ openTab, setOpenTab ] = useState('categoriesListTab');
    ...
    const handleClick = (e) => {
        switch(e.target.id) {
            case "categoriesListTab":
                setOpenTab("categoriesListTab");
            break;
            case "addProdTab":
    ...
    const handleModal = (e, data) => {
        switch (e.currentTarget.ariaLabel) {
            ...
            case 'editProdModal':
                ...
                break;
            case 'confirmDelCategory':
                setShowModal("confirmDelCategory");
                setCategoriesObj(data);
                break;
            case 'addCategoryModal':
                setShowModal("addCategoryModal");
                break;
            case 'editCategoryModal':
                setShowModal("editCategoryModal");
                setCategoriesObj(data);
                break;
        }
    }
    ...
    const fetchInsertProd = async (prodModel) => {
     ...
     const fetchDelCategory = async (category_id) => {
        await axiosPrivate.delete("/category", {params: {id: category_id}})
        .then(response => {
            console.log(response)
            if(response.data == 1){
                toast.current.show({
        ...
        .catch(error => {
            ...
            toast.current.show({
                severity: "error",
                summary: "Failed",
                detail: "Error when deleting category",
                life: 3000,
            });
        })
    };
    ...
    useEffect(() => {
        if(delCategory){
            fetchDelCategory(categoriesObj.id);
        }
    },[delCategory]);
    ...
    return(
        ...
        {/* <Sidebar show={isClose} /> */}
                    ...
                    <div className="row mt-4">
                        <div className="col-lg-12 col-sm-12 col-md-12 col-12">
                            <div className="basic-tabs">
                                <div className="tabs">
                                    <div className={`tab-indicator ${openTab === "categoriesListTab" ? "active" : ""}`}  
                                        id='categoriesListTab' 
                                        onClick={(e) => handleClick(e)}
                                    >
                                        <span className="tab-title">Kategori</span>
                                    </div>
                                </div>
                                <div className="tabs-content" style={openTab === "categoriesListTab" ? {display: "block"} : {display: "none"}}>
            ...
            {showModal === "salesDetailModal" ? 
                (
                    <SalesDetailModal show={showModal === "salesDetailModal" ? true : false} onHide={handleCloseModal} data={showModal == "salesDetailModal" ? categoriesObj : ""} />
                )
             : showModal === "editCategoryModal" ?
                (
                    <EditCategoryModal show={showModal == "editCategoryModal" ? true : false} onHide={handleCloseModal}  categoryList={showModal === "editCategoryModal" ? categoryList :''} data={showModal == "editCategoryModal" ? categoriesObj : ""} />
                )
             : showModal === "confirmDelCategory" ? (
                    <ConfirmModal
                      show={showModal === "confirmDelCategory" ? true : false}
                      onHide={handleCloseModal}
                      data={showModal === "confirmDelCategory" ? categoriesObj : ""}
                      msg={"Yakin ingin menghapus kategori ini?"}
                      returnValue={(value) => {setDelCategory(value)}}
                    />
            ): showModal === "addCategoryModal" ? (
                    <AddCategoryModal  
                        show={showModal === "addCategoryModal" ? true : false}
                        onHide={handleCloseModal}
                        categoryList={showModal === "addCategoryModal" ? categoryList :''}
                    />
            ) : showModal === "warningCancelModal" ?
                (
                    <ConfirmModal show={showModal === "warningCancelModal" ? true : false} onHide={handleCloseModal} 
                        data={showModal === "warningCancelModal" ? categoriesObj : ""} 
                        msg={
             ...
             : showModal === "existInvOrderModal" ?
                (
                    <ConfirmModal show={showModal === "existInvOrderModal" ? true : false} onHide={handleCloseModal} 
                        data={showModal === "existInvOrderModal" ? categoriesObj : ""} 
                        msg={
                            <p style={{marginBottom: 0}}>
            ...
            : showModal === "confirmationModal" ?
                (
                    <ConfirmModal show={showModal === "confirmationModal" ? true : false} onHide={handleCloseModal} 
                        data={showModal === "confirmationModal" ? categoriesObj : ""}  
                        msg={modalMsg}
